...
// Mock utils module
vi.doMock("../utils", () => ({
  SUPPORTED_FILES: [".vue", ".svelte", ".ts", ".tsx", ".js", ".jsx", ".html", ".blade.php"],
  ...
  shouldProcessFile: vi.fn().mockImplementation((filePath: string) => {
    ...
    const supportedFiles = [".vue", ".svelte", ".ts", ".tsx", ".js", ".jsx", ".html", ".blade.php"];
    return (
      supportedFiles.some((ext) => filePath?.endsWith(ext)) &&
...
vi.doMock("../core", () => ({
  ...
  transformClassModifiers: vi.fn((code) => {
    return code
      .replace(/class:hover="([^"]*)"/g, 'class="hover:$1"')
...
describe("useClassy plugin", () => {
    ...
    it("should handle WebSocket notifications in dev mode", async () => {
      ...
      if (plugin.configureServer && typeof plugin.configureServer === "function") {
        plugin.configureServer(
          mockServer as unknown as Parameters<typeof plugin.configureServer>[0],
        );
      }
  ...
  describe("Build Lifecycle Hooks", () => {
    ...
    it("should inject Tailwind @source into stylesheets that import Tailwind", async () => {
      ...
      const result = transform(css, "/project/app/assets/main.css");
