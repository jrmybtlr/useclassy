...
export const REACT_CLASS_MODIFIER_REGEX = /(?<![:\w])(?:className|class):([\w-:]+)="([^"]*)"/g;
...
/**
 * Split a whitespace-separated class string into individual tokens.
 */
export function tokenizeClassValues(value: string): string[] {
  const tokens: string[] = [];
  let start = 0;
  ...
  for (let i = 0; i <= len; i++) {
    ...
    if (char === " " || char === "\t" || char === "\n" || i === len) {
      if (i > start) {
        const cls = value.substring(start, i);
        if (cls) tokens.push(cls);
      }
      while (i < len && /\s/.test(value[i + 1] ?? "")) i++;
      start = i + 1;
    }
  }
...
function addClassTokens(classStr: string, allFileClasses: Set<string>): void {
  for (const cls of tokenizeClassValues(classStr)) allFileClasses.add(cls);
}
...
export function extractClasses(
  ...
  while ((classMatch = classRegex.exec(code)) !== null) {
    ...
    if (staticClasses) {
      addClassTokens(staticClasses, allFileClasses);
    }
    ...
    if (jsxClasses) {
      ...
      if (trimmedJsx.startsWith("`") && trimmedJsx.endsWith("`")) {
        ...
        if (staticPart) {
          addClassTokens(staticPart, allFileClasses);
        }
      }
    }
  }
  ...
  while ((modifierMatch = classModifierRegex.exec(code)) !== null) {
    ...
    if (modifiers && classes) {
      for (const cls of tokenizeClassValues(classes)) {
...
export function transformClassModifiers(
  ...
  return code.replace(classModifierRegex, (match, modifiers, classes) => {
    ...
    for (const raw of tokenizeClassValues(classes)) {
      const value = raw.trim();
      if (!value) continue;
      modifiedClassesArr.push(`${modifiers}:${value}`);
      if (modifierParts.length > 1) {
        for (const part of modifierParts) {
          if (part) modifiedClassesArr.push(`${part}:${value}`);
        }
      }
    }
...
function getCompiledRegexes(attrName: string): {
  ...
  if (!cached) {
    cached = {
      ...
      attrFinderRegex: new RegExp(`(?:${attrName}|class)=(?:(?:"([^"]*)")|(?:{([^}]*)}))`, "g"),
    };
...
export function mergeClassAttributes(code: string, attrName: string): string {
  ...
  return code.replace(multipleClassRegex, (match) => {
    ...
    if (jsxExpr) {
      if (!combinedStatic) return `${attrName}={${jsxExpr}}`;
