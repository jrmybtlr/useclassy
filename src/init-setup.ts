...
function readPackageJson(cwd: string): Record<string, unknown> | null {
  ...
  try {
    return parseJsonRecord(fs.readFileSync(packagePath, "utf-8"));
  } catch {
    return null;
  }
}
...
function mergedDependencies(pkg: Record<string, unknown>): Record<string, string> {
  return {
    ...
    ...readStringRecord(pkg.devDependencies),
  };
}
...
function tailwindConfigExists(cwd: string): boolean {
  return TAILWIND_CONFIG_NAMES.some((name) => fs.existsSync(path.join(cwd, name)));
}
...
function addRootCssFiles(projectRoot: string, out: string[]): void {
  ...
  for (const ent of entries) {
    ...
    if (/\.(css|pcss)$/i.test(ent.name)) out.push(path.join(projectRoot, ent.name));
  }
}
...
function ensureUseClassyImport(content: string): string {
  if (/from\s+["']vite-plugin-useclassy["']/.test(content)) return content;
...
function insertUseClassyPlugin(content: string, language: InitLanguage): string {
  if (/useClassy\s*\(/.test(content)) return content;

  const pluginBlock = `useClassy({\n      language: '${language}',\n    }),`;

  const pluginsMatch = content.match(/plugins\s*:\s*\[/);
  if (!pluginsMatch || pluginsMatch.index === undefined) {
    throw new Error("Could not find a `plugins: [` array in vite.config. Add useClassy manually.");
  }
...
export function patchViteConfigContent(content: string, language: InitLanguage): string {
...
function applyTextFilePatch(
  ...
  if (next === original) return { path: file, changed: false };
...
export function patchViteConfig(
  ...
  return applyTextFilePatch(file, (content) => patchViteConfigContent(content, language), dryRun);
}
...
export function patchTailwindV4Stylesheet(
  ...
  if (manifestReferencedIn(original)) return { path: cssFile, changed: false };
  ...
  if (insertIndex === -1) return { path: cssFile, changed: false };

  const nextLines = [...lines];
  nextLines.splice(insertIndex, 0, sourceLine);
  const next = nextLines.join("\n");

  if (!dryRun) fs.writeFileSync(cssFile, next, "utf-8");

  return { path: cssFile, changed: true };
}

export function patchTailwindV4(cwd: string, dryRun: boolean): FilePatchResult {
  const files = findTailwindCssEntryFiles(cwd);
  if (files.length === 0) {
    return {
      path: "",
      changed: false,
      error: 'No CSS file with @import "tailwindcss" found. Add @source manually (see README).',
    };
  }
...
export function patchTailwindV3ConfigContent(content: string): string {
  ...
  if (content.includes("output.classy.html")) return content;
  ...
  return content.replace(/(content:\s*\[)/, `$1\n    "${entry}",`);
}

export function patchTailwindV3(cwd: string, dryRun: boolean): FilePatchResult {
...
export function patchVsCodeSettings(
  ...
  if (fs.existsSync(file)) {
    const parsed = parseJsonRecord(fs.readFileSync(file, "utf-8"));
    if (!parsed) {
      return {
        path: file,
        changed: false,
...
function pushViteMessages(result: InitSetupResult, vite: FilePatchResult, dryRun: boolean): void {
  if (vite.error) {
...
function pushVsCodeMessages(result: InitSetupResult, vs: FilePatchResult, dryRun: boolean): void {
  if (vs.error) {
