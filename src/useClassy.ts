...
export default function useClassy(options: ClassyOptions = {}): Plugin {
  ...
  // Use appropriate regex patterns based on framework
  const classRegex = isReact ? REACT_CLASS_REGEX : CLASS_REGEX;
  const classModifierRegex = isReact ? CLASS_MODIFIER_REGEX : CLASS_MODIFIER_REGEX;
  ...
  function mergeClassAttributes(code: string): string {
    return code.replace(multipleClassRegex, (match) => {
      const allClasses = match
        .match(new RegExp(`${classAttrName}="([^"]*)"`, 'g'))
        ?.map(cls => {
          const subMatch = cls.match(new RegExp(`${classAttrName}="([^"]*)"`));
          return subMatch ? subMatch[1] : '';
        })
        .filter(Boolean)
        .join(' ') || '';

      return `${classAttrName}="${allClasses}"`;
    });
  }
}
