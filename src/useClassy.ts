...
const SUPPORTED_FILES = [".vue", ".ts", ".tsx", ".js", ".jsx", ".html"];
...
function writeOutputFile(
  ...
  try {
    ...
    export default function ClassyOutput() {
      return (
        <div>
          ${Array.from(allClassesSet)
            .map((cls) => `<div ${classAttrName}="${cls}" />`)
            .join("\n      ")}
        </div>
      );
    }`;
...
export default function useClassy(options: ClassyOptions = {}): Plugin {
  ...
  const multipleClassRegex = isReact
    ? REACT_MULTIPLE_CLASS_REGEX
    : MULTIPLE_CLASS_REGEX;
  const classAttrName = isReact ? "className" : "class";
  ...
  function processCode(code: string, allClassesSet: Set<string>): string {
    ...
    return mergeClassAttributes(
      transformClassModifiers(code, generatedClassesSet),
      classAttrName
    );
  }
   ...
   */
  function transformClassModifiers(
    code: string,
    generatedClassesSet: Set<string>
  ): string {
